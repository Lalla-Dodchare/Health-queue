...
/**
 * Create appointment approved notification (with Email + SMS)
...
export async function createAppointmentApprovedNotification(appointment, userProfile = null, supabaseClient = null) {
  ...
  // Send Email if user profile has email
  if (userProfile?.email) {
    await sendAppointmentApprovedEmail({
      ...
      doctorName: appointment.doctor_name || 'แพทย์',
      branchName: appointment.branch_name || 'สาขา'
    })
  }
...
/**
 * Create appointment rejected notification (with Email + SMS)
 * @param {Object} appointment - Appointment data
...
export async function createAppointmentRejectedNotification(appointment, reason = null, userProfile = null, supabaseClient = null) {
  ...
  const formattedPrimaryDate = new Date(appointment.primary_date).toLocaleDateString('th-TH', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    weekday: 'long'
  ...
  let formattedSecondaryDate = null
  if (appointment.secondary_date) {
    formattedSecondaryDate = new Date(appointment.secondary_date).toLocaleDateString('th-TH', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
      weekday: 'long'
...
/**
 * Create appointment created notification (in-app only, NO Email/SMS)
 * Sent immediately when user creates a new appointment
 * Email and SMS will be sent ONLY when admin approves the appointment
 * @param {Object} appointment - Appointment data
 * @param {Object} userProfile - User profile data (optional)
 * @param {Object} supabaseClient - Supabase client to use (optional)
 */
export async function createAppointmentCreatedNotification(appointment, userProfile, supabaseClient = null) {
  // Format dates for display
  const formattedPrimaryDate = new Date(appointment.primary_date).toLocaleDateString('th-TH', {
    year: 'numeric',
    month: 'long',
  ...
  // Create in-app notification ONLY (no Email/SMS)
  const notifResult = await createNotification({
    userId: appointment.user_id,
    title: '📋 ยืนยันการจองนัดหมาย',
    message: `การจองของคุณส่งเรียบร้อยแล้ว รอการอนุมัติ\nตัวเลือกที่ 1: ${formattedPrimaryDate} ${appointment.primary_time}${
      formattedSecondaryDate ? `\nตัวเลือกที่ 2: ${formattedSecondaryDate} ${appointment.secondary_time}` : ''
    }`,
    type: 'appointment',
    priority: 'normal',
    relatedAppointmentId: appointment.id,
  ...
  // NOTE: Email and SMS are NOT sent here
  // They will be sent when admin approves via createAppointmentApprovedNotification()
