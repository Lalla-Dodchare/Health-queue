...
/**
 * Send appointment created/booked SMS
 * Sent when user successfully creates a new appointment
 */
export async function sendAppointmentCreatedSMS({
  phone,
  patientName,
  primaryDate,
  primaryTime,
  ...
  let message = `สวัสดีค่ะคุณ ${patientName}\n\n📋 ยืนยันการจองนัดหมาย\n\n✅ การจองส่งเรียบร้อยแล้ว รอการอนุมัติ`

  if (doctorName) {
    message += `\n\n👨‍⚕️ แพทย์: ${doctorName}`
  }

  message += `\n📅 ตัวเลือกที่ 1: ${primaryDate} ${primaryTime}`

  if (secondaryDate && secondaryTime) {
    message += `\n📅 ตัวเลือกที่ 2: ${secondaryDate} ${secondaryTime}`
  }

  message += `\n\nเจ้าหน้าที่จะอนุมัติภายใน 24 ชม.\n\n- Health Queue`
